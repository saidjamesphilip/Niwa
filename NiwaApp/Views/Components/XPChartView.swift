...
struct XPChartView: View {
    ...
    private var last7DaysData: [(day: String, task: Int, timer: Int, note: Int, water: Int, stand: Int)] {
    ...
    private var maxDayTotal: Int {
        max(last7DaysData.map { $0.task + $0.timer + $0.note + $0.water + $0.stand }.max() ?? 1, 1)
    }
                ...
                ForEach(Array(last7DaysData.enumerated()), id: \.offset) { _, day in
                    VStack(spacing: 2) {
                        ...
                        VStack(spacing: 0) {
                            ...
                            barSegment(value: day.task, color: DesignTokens.Colors.primary)
                        }
                        .frame(height: 60)
    ...
    private func barSegment(value: Int, color: Color) -> some View {
        Rectangle()
            .fill(color)
            .frame(height: max(0, CGFloat(value) / CGFloat(maxDayTotal) * 60))
    }
}
